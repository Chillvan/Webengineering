...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->Cell(10);
        // Title
        $this->Cell(0,20,'Bezahlte Rechnungen Mehrfamilienhaus',0,0,'C');
        // Line break
        $this->Ln(20);
    }
    ...
    function fetchData() {
        ...
        try {
            ...
            $query = $dbh->prepare('SELECT * FROM Rechnungen, Mieterspiegel WHERE Mieterspiegel.Mieternummer=Rechnungen.Mieternummer AND Rechnungen.Bezahlt="1" ORDER BY Rechnungsnummer ASC');
            $query->execute();
    ...
    function fetchGesamt() {
        ...
        try {
            $dbh = new PDO($host, $user, $pass);
            $sth = ($dbh->query('SELECT SUM(Betrag) FROM Rechnungen WHERE Rechnungen.Bezahlt="1"'));
